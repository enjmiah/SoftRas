...
__device__ __forceinline__ scalar_t forward_sample_depth_texture(
    ...
    for (int i = 0; i < texture_width; i += 3) { // go by twos to speed up a little
        for (int j = 0; j < texture_width; j += 3) {
    ...
    if (shadow_map) {
        ...
        if (s_x < 0 || s_y < 0 || s_x >= shadow_map_size || s_y >= shadow_map_size) {
            ...
            //const auto light_closest = forward_sample_texture<scalar_t>(
                //shadow_map, shadow_map_size, shadow_u, 1.0 - shadow_v, sigma, 0, -10000, 10);
            ...
            shadow_term = forward_sample_depth_texture<scalar_t>(
                shadow_map, shadow_map_size, shadow_u, 1.0 - shadow_v, sigma, light_depth-bias, sharpness);
        }
    }
